...
// .wrangler/tmp/bundle-SWAROo/checked-fetch.js
...
// .wrangler/tmp/bundle-SWAROo/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-SWAROo/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
async function searchShowbox(keyword, type = "all") {
...
__name(searchShowbox, "searchShowbox");
async function getFebBoxId(id, type) {
  const url = `https://www.showbox.media/index/share_link?id=${id}&type=${type}`;
  const res = await fetch(url);
  const data2 = await res.json();
  return data2?.data?.link?.split("/").pop() || null;
}
__name(getFebBoxId, "getFebBoxId");
async function getFebBoxLinks(fid) {
  ...
  const data2 = await res.json();
  const html3 = data2.html || "";
  const $2 = load(html3.includes("<tr") ? `<table>${html3}</table>` : html3);
  const results = [];
  $2("tr").each((_, row) => {
    const $row = $2(row);
    const quality = $row.find("td").first().text().trim();
    const url2 = $row.find("a.btn-download").attr("href");
    if (quality && url2) {
      results.push({
        url: url2,
        quality,
  ...
  $2(".file_quality").each((_, div) => {
    const $div = $2(div);
    const url2 = $div.attr("data-url");
...
__name(getFebBoxLinks, "getFebBoxLinks");
app.get("/resolve/movie", async (c) => {
  ...
  try {
    ...
    const match2 = results.find(
      (r) => r.title.toLowerCase() === title.toLowerCase() && (!year || r.year?.toString() === year)
    ) || results[0];
    if (!match2) return c.json({ error: "Movie not found" }, 404);
    const febBoxId = await getFebBoxId(match2.id, "1");
    ...
    const videoFiles = (flData.data?.file_list || []).filter(
      (f) => f.is_dir === 0 && (f.file_name.endsWith(".mp4") || f.file_name.endsWith(".mkv"))
    );
