...
(async () => {
  ...
  if (febBoxId) {
    ...
    const file = files[1];
    const links = await febboxApi.getLinks(file.id, febBoxId);
  ...
  if (febBoxId) {
    ...
    if (file.is_dir) {
      ...
      const links = await febboxApi.getLinks(seasonFile.id, febBoxId);
  ...
  console.log("🧭 Discover Categories:", discoverCategories);
})();
